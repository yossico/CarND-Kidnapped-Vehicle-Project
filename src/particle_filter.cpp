...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	/* TODO: 
	1) Set the number of particles. - 200
	2) Initialize all particles to first position (based on x, y, theta and noise from GPS) set init weights to 1. 
	...
	// init num_particles of particles
	for (int i = 0; i < num_particles; i++) {
		//Initialize all particles to first position+ weights
		Particle p;
		p.id = i;
		p.x = x;
		p.y = y;
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	//x,y and theta normal distributions
	...
	for (int i = 0; i < num_particles; i++) {
		...
		// calculate new state
		if (fabs(yaw_rate) < EFES) { //avoid zero value issues
